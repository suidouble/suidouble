...
class SuiPackage extends SuiObject {
    ...
    async getModulesNamesFromBuild() {
        ...
        try {
            const buildPathContent = await fs.promises.readdir(path.join(this._path, 'build'));
    
            this.log('build path contents: ', buildPathContent);
    
            // @todo: there may be some junk folders and we'd have to get project name from Move.toml ?
            const buildPath = buildPathContent[0];
    
            const dirents = await fs.promises.readdir(path.join(this._path, 'build', buildPath, 'bytecode_modules'), { withFileTypes: true });
            const names = dirents
                .filter(dirent => dirent.isFile())
                .map(dirent => dirent.name.split('.mv').join(''));
    
            this.log('modules names', names);
